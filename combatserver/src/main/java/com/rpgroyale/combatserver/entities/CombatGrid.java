...
public class CombatGrid {
    ...
    private static CombatGrid _instance = null;
        ...
        for(int x = -1; x < 2; x++) {
            for (int y = -1; y < 2; y++) {
                ...
                if (isValidLocation(neighborNode.x, neighborNode.y)) {
                    GridLocation neigborLocation = getLocationAt(neighborNode.x, neighborNode.y);
    ...
    {
        Boolean invalid = (locX < 0) || (locY < 0) || (locX >= width) || (locY >= height);
        return !invalid;
    }
